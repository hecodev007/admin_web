...
Main.page-exchange
        ...
        Form(:model='createForm', :label-width='100')
            ...
            FormItem(:label='$lang("合约地址")' style="width:400px" v-if= 'tokenType != 4 && tokenType != 5')
            ...
            FormItem(:label='$lang("gas限制")' style="width:400px" v-if= 'tokenType != 2 && tokenType != 4 && tokenType != 5 && tokenType != 6')
                Input(v-model='createForm.gas_limit',  :placeholder="$lang('请输入',$lang('gas限制'))")

            FormItem(:label='$lang("可交易确认数")' style="width:400px" v-if= 'tokenType == 4')
                Input(v-model='createForm.can_transaction_confirms', , :placeholder="$lang('请输入',$lang('可交易确认数'))")
            FormItem(:label='$lang("可提现确认数")' style="width:400px" v-if= 'tokenType == 4')
                Input(v-model='createForm.can_withdrawal_confirms', , :placeholder="$lang('请输入',$lang('可提现确认数'))")
            FormItem(:label='$lang("消耗币种")' style="width:400px" v-if= 'tokenType == 1 ||tokenType == 4 ||  tokenType == 5')
                Input(v-model='createForm.fee_token_id',  :placeholder="$lang('请输入',$lang('消耗币种'))")
            FormItem(:label='$lang("扩展字段")' style="width:400px" v-if= 'tokenType == 2 || tokenType == 4')
                Input(v-model='createForm.extension', , :placeholder="$lang('请输入',$lang('扩展字段'))")
            FormItem(:label='$lang("归集触发值")' style="width:400px" v-if= 'tokenType == 1 || tokenType == 6')
                Input(v-model='createForm.collect_threshold')

            FormItem(:label='"api_key"' style="width:400px" v-if= 'tokenType == 5')
                Input(v-model='createForm.api_key')
            FormItem(:label='"ed25519_publickey"' style="width:400px" v-if= 'tokenType == 5')
                Input(v-model='createForm.ed25519_publickey')
            FormItem(:label='$lang("主链")' style="width:400px" v-if= 'tokenType == 5')
                Input(v-model='createForm.chain')
            FormItem(:label='$lang("主链类型")' style="width:400px" v-if= 'tokenType == 5')
                Input(v-model='createForm.chain_type')
            FormItem(:label='$lang("地址正则")' style="width:400px" v-if= 'tokenType == 5')
                Input(v-model='createForm.address_regexp')
            FormItem(:label='$lang("memo正则")' style="width:400px" v-if= 'tokenType == 5')
                Input(v-model='createForm.memo_regexp')
            FormItem(:label='"NeedMemo"' style="width:400px" v-if= 'tokenType == 5')
                Input(v-model='createForm.need_memo')
            FormItem(:label='$lang("归集地址")' style="width:400px" v-if= 'tokenType == 5')
