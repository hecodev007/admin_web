<template lang="pug">
  Main.page-exchange
    ...
    Form(:model='createForm', :label-width='150')
      ...
      FormItem(:label='$lang("gas限制")' style="width:400px"   v-if= 'tokenType ==0')
        Input(v-model='createForm.gas_limit',  :placeholder="$lang('请输入',$lang('gas限制'))")

      FormItem(:label='$lang("可交易确认数")' style="width:400px"  v-if= 'tokenType ==0')
        Input(v-model='createForm.can_transaction_confirms', , :placeholder="$lang('请输入',$lang('可交易确认数'))")
      FormItem(:label='$lang("可提现确认数")' style="width:400px"  v-if= 'tokenType ==0')
        Input(v-model='createForm.can_withdrawal_confirms', , :placeholder="$lang('请输入',$lang('可提现确认数'))")
      FormItem(:label='$lang("消耗币种")' style="width:400px"  v-if= 'tokenType ==0')
      ...
      FormItem(:label='$lang("地址正则")' style="width:400px"  v-if= 'tokenType ==0')
      ...
      FormItem(:label='"NeedMemo"' style="width:400px"  v-if= 'tokenType ==0')
        Input(v-model='createForm.need_memo')
      FormItem(:label='$lang("归集地址")' style="width:400px"  v-if= 'tokenType ==0')
