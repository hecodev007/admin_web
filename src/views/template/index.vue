...
Main.page-exchange
    Row.search(:grtter="15")
      Col(span="4")
        Button(size="large" type="primary" @click="create" icon="md-add-circle" long) {{$lang('新建')}}
    Table.user-table(:columns="columns", :data="listData", :loading="loadingTable")

    // 创建修改券商    
    Modal.page-exchange-win(v-model="creatWinStatus", :title="$lang(createForm.brokerId ? '修改':'新建')" @on-ok="doNext")
      Form(:model='createForm', :label-width='100')
        FormItem(:label="$lang('券商')")
          Input(v-model='createForm.brokerName', :placeholder="$lang('请输入',$lang('券商'))", :disabled="createForm.brokerId")
        FormItem(:label="$lang('签名')")
          Input(v-model='createForm.sign', :placeholder="$lang('请输入',$lang('请输入短信签名'))")
</template>
<script>
export default {
  data() {
      ...
      this.$axios
        ...
        .then(result => {
          if (result.code === 0) {
            this.$Notice.success({
              title: this.$lang(
                this.createForm.brokerId ? "修改成功" : "添加成功"
              )
            });
          }
          this.loadData();
