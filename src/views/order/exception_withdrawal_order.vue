<template lang="pug">
  Main.page-exchange
    ...
    Modal.page-exchange-detail(v-model="detialWinSatus", :title="createForm.token_id")
      ...
      Row
        Col(span="6" style="text-align:left") Org_ID
        Col(span="14") {{createForm.org_id}}
      Row
        Col(span="6" style="text-align:left") Nonce
        Col(span="14") {{createForm.nonce}}
      Row
...
export default {
  ...
  data() {
    return {
      ...
      resendTypeList: [
        ...
        {
          key: 2,
          value: 'withdrawal refund，notify=false'
        },
        {
          key: 3,
          value: 'withdrawal refund，notify=true'
        },
        {
          key: 4,
          value: 'onChainFailed'
  ...
  methods: {
    loadData() {
      ...
      this.$axios.get("/api/v1/exception_orders", {params: this.requestDataForm}).then(result => {
